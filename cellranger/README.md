...
ln -s /lustre/home/acct-medcl/share /lustre/home/acct-medcl/wyyang2025/workspace/share_by_teacherCL#语法：ln -s 原文件目录 目的文件目录

#记录一下10X illumina的下机数据是什么样的，首先是两个个fastq文件，read1文件是barcode+UMI；read2文件是5'端开始的序列信息。每个文件的第一行是文件名字，第二行是碱基序列，第三行记录是正义链（+）还是反义链（-），第四行记录的质量信息
